  ...
  implements X500NameStyle
{
  ...
  {
    if (paramBoolean) {
      for (int j = -1 + paramArrayOfRDN.length; j >= 0; j--) {
  ...
  public int calculateHashCode(X500Name paramX500Name)
  {
    ...
    for (int j = 0; j != arrayOfRDN.length; j++)
    {
      if (arrayOfRDN[j].isMultiValued())
      {
        AttributeTypeAndValue[] arrayOfAttributeTypeAndValue = arrayOfRDN[j].getTypesAndValues();
        for (int k = 0; k != arrayOfAttributeTypeAndValue.length; k++) {
  ...
  public RDN[] fromString(String paramString)
  {
    ...
    for (int i = 0; i != arrayOfRDN1.length; i++) {
  ...
  public String toString(X500Name paramX500Name)
  {
    ...
    int i = 1;
    RDN[] arrayOfRDN = paramX500Name.getRDNs();
    for (int j = -1 + arrayOfRDN.length; j >= 0; j--)
    {
      ...
      label115:
      IETFUtils.appendTypeAndValue(localStringBuffer, arrayOfRDN[j].getFirst(), DefaultSymbols);
    }
