  ...
  implements BlockCipher
{
    ...
    throws IllegalArgumentException
  {
    ...
    if (i == 0)
    {
      ...
      arrayOfInt2 = new int[''];
      for (int m = 8; m < 16; m++) {
        arrayOfInt1[m] = rotateLeft(0x9E3779B9 ^ arrayOfInt1[(m - 8)] ^ arrayOfInt1[(m - 5)] ^ arrayOfInt1[(m - 3)] ^ arrayOfInt1[(m - 1)] ^ m - 8, 11);
      }
    }
    throw new IllegalArgumentException("key must be a multiple of 4 bytes");
    System.arraycopy(arrayOfInt1, 8, arrayOfInt2, 0, 8);
    for (int n = 8; n < 132; n++) {
      arrayOfInt2[n] = rotateLeft(n ^ 0x9E3779B9 ^ arrayOfInt2[(n - 8)] ^ arrayOfInt2[(n - 5)] ^ arrayOfInt2[(n - 3)] ^ arrayOfInt2[(n - 1)], 11);
    }
