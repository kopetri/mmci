  ...
  implements AEADBlockCipher
{
  ...
  private void gCTRBlock(byte[] paramArrayOfByte1, int paramInt1, byte[] paramArrayOfByte2, int paramInt2)
  {
    for (int i = 15; i >= 12; i--)
    {
      int k = (byte)(0xFF & 1 + this.counter[i]);
      this.counter[i] = k;
      if (k != 0) {
        break;
      }
    ...
    for (byte[] arrayOfByte2 = arrayOfByte1;; arrayOfByte2 = paramArrayOfByte1) {
      for (int j = paramInt1 - 1; j >= 0; j--)
      {
        arrayOfByte1[j] = ((byte)(arrayOfByte1[j] ^ paramArrayOfByte1[j]));
        paramArrayOfByte2[(paramInt2 + j)] = arrayOfByte1[j];
      }
    }
  ...
  {
    byte[] arrayOfByte1 = new byte[16];
    for (int i = 0; i < paramArrayOfByte.length; i += 16)
    {
    ...
    throws IllegalArgumentException
  {
    ...
    if ((paramCipherParameters instanceof AEADParameters))
    {
      ...
      this.macSize = (j / 8);
      localKeyParameter = localAEADParameters.getKey();
      if (!paramBoolean) {
        break label206;
      }
    }
    label206:
    for (int i = 16;; i = 16 + this.macSize)
    {
      ...
      if ((paramCipherParameters instanceof ParametersWithIV))
      {
        ...
        this.A = null;
        this.macSize = 16;
        localKeyParameter = (KeyParameter)localParametersWithIV.getParameters();
        break;
      }
