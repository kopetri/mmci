...
public class TlsProtocolHandler
{
    ...
    throws IOException
  {
    for (;;)
    {
      ...
      if (this.alertQueue.size() >= 2)
      {
        ...
        int i = (short)arrayOfByte[0];
        j = (short)arrayOfByte[1];
        if (i == 2)
        {
          this.failedWithError = true;
          this.closed = true;
    ...
    throws IOException
  {
    ...
    do
    {
      ...
      for (;;)
      {
        ...
        return;
        Certificate localCertificate = Certificate.parse(localByteArrayInputStream1);
        assertEmpty(localByteArrayInputStream1);
        this.keyExchange.processServerCertificate(localCertificate);
        this.authentication = this.tlsClient.getAuthentication();
        this.authentication.notifyServerCertificate(localCertificate);
      }
      switch (this.connection_state)
      {
      default: 
      ...
      for (int i1 = 12;; i1 = 36)
      {
        byte[] arrayOfByte8 = new byte[i1];
        TlsUtils.readFully(arrayOfByte8, localByteArrayInputStream1);
        assertEmpty(localByteArrayInputStream1);
        if (!Arrays.constantTimeAreEqual(TlsUtils.calculateVerifyData(this.tlsClientContext, "server finished", this.rs.getCurrentHash(TlsUtils.SSL_SERVER)), arrayOfByte8)) {
          failWithError((short)2, (short)40);
        }
      ...
      ProtocolVersion localProtocolVersion1 = TlsUtils.readVersion(localByteArrayInputStream1);
      ProtocolVersion localProtocolVersion2 = this.tlsClientContext.getClientVersion();
      if (localProtocolVersion1.getFullVersion() > localProtocolVersion2.getFullVersion()) {
        failWithError((short)2, (short)47);
      }
      this.tlsClientContext.setServerVersion(localProtocolVersion1);
      this.tlsClient.notifyServerVersion(localProtocolVersion1);
      ...
      int m = TlsUtils.readUint16(localByteArrayInputStream1);
      if ((!arrayContains(this.offeredCipherSuites, m)) || (m == 255)) {
        failWithError((short)2, (short)47);
      }
      this.tlsClient.notifySelectedCipherSuite(m);
      ...
      {
        ByteArrayInputStream localByteArrayInputStream3 = new ByteArrayInputStream(TlsUtils.readOpaque16(localByteArrayInputStream1));
        while (localByteArrayInputStream3.available() > 0)
        {
      ...
      for (;;)
      {
        ...
        byte[] arrayOfByte2 = this.keyExchange.generatePremasterSecret();
        this.securityParameters.masterSecret = TlsUtils.calculateMasterSecret(this.tlsClientContext, arrayOfByte2);
        Arrays.fill(arrayOfByte2, (byte)0);
        if ((localTlsCredentials != null) && ((localTlsCredentials instanceof TlsSignerCredentials)))
        {
          sendCertificateVerify(((TlsSignerCredentials)localTlsCredentials).generateCertificateSignature(this.rs.getCurrentHash(null)));
          this.connection_state = 8;
        }
        byte[] arrayOfByte3 = { 1 };
        this.rs.writeMessage((short)20, arrayOfByte3, 0, arrayOfByte3.length);
        this.connection_state = 9;
        this.rs.clientCipherSpecDecided(this.tlsClient.getCompression(), this.tlsClient.getCipher());
        byte[] arrayOfByte4 = TlsUtils.calculateVerifyData(this.tlsClientContext, "client finished", this.rs.getCurrentHash(TlsUtils.SSL_CLIENT));
        ByteArrayOutputStream localByteArrayOutputStream = new ByteArrayOutputStream();
        ...
        else
        {
          ...
          sendClientCertificate(localTlsCredentials.getCertificate());
        }
      }
      switch (this.connection_state)
      {
      ...
      for (;;)
      {
        ...
        ByteArrayInputStream localByteArrayInputStream2 = new ByteArrayInputStream(arrayOfByte1);
        while (localByteArrayInputStream2.available() > 0) {
          localVector.addElement(X500Name.getInstance(ASN1Primitive.fromByteArray(TlsUtils.readOpaque16(localByteArrayInputStream2))));
        }
    ...
    throws IOException
  {
    ...
    TlsUtils.writeUint24(0, localByteArrayOutputStream);
    paramCertificate.encode(localByteArrayOutputStream);
    byte[] arrayOfByte = localByteArrayOutputStream.toByteArray();
    TlsUtils.writeUint24(-4 + arrayOfByte.length, arrayOfByte, 1);
    this.rs.writeMessage((short)22, arrayOfByte, 0, arrayOfByte.length);
  }
    ...
    throws IOException
  {
    ...
    this.tlsClientContext.setClientVersion(localProtocolVersion);
    this.tlsClientContext.setServerVersion(localProtocolVersion);
    TlsUtils.writeVersion(localProtocolVersion, localByteArrayOutputStream1);
    ...
    for (int i = 1;; i = 0)
    {
      ...
      Enumeration localEnumeration = this.clientExtensions.keys();
 ...
 * Qualified Name:     org.spongycastle.crypto.tls.TlsProtocolHandler
