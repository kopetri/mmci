  ...
  public BackStackRecord instantiate(FragmentManagerImpl paramFragmentManagerImpl)
  {
    ...
    for (int j = 0; i < this.mOps.length; j++)
    {
      ...
      if (FragmentManagerImpl.DEBUG) {
        new StringBuilder("Instantiate ").append(localBackStackRecord).append(" op #").append(j).append(" base fragment #").append(this.mOps[k]).toString();
      }
      int[] arrayOfInt2 = this.mOps;
      int m = k + 1;
      int n = arrayOfInt2[k];
      ...
      for (localOp.fragment = ((Fragment)paramFragmentManagerImpl.mActive.get(n));; localOp.fragment = null)
      {
        ...
        {
          if (FragmentManagerImpl.DEBUG) {
            new StringBuilder("Instantiate ").append(localBackStackRecord).append(" set remove fragment #").append(this.mOps[i5]).toString();
          }
